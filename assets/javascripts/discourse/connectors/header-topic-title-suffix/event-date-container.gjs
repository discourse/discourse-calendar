...
export default class EventDateContainer extends Component {
    static shouldRender({ topic }, { siteSettings }) {
        return siteSettings.discourse_post_event_enabled && topic.event_starts_at && topic.event_ends_at;
    }
    ...
    <template>
        <div class="header-topic-title-suffix-outlet event-date-container">
            {{#if this.siteSettings.use_local_event_date}}
                <span class="event-date event-local-date past" data-starts_at={{this.eventStartedAt}} title={{this.dateRange}} data-ends_at={{this.eventEndedAt}}>
                    {{this.localDateContent}}
                </span>
            {{else}}
                <span class="event-date event-relative-date {{this.relativeDateType}}" title={{this.dateRange}} data-starts_at={{this.eventStartedAt}} data-ends_at={{this.eventEndedAt}}>
    ...
    get eventStartedAt() {
        return this._parsedDate(this.args.outletArgs.topic.event_starts_at);
    }

    get eventEndedAt() {
        return this._parsedDate(this.args.outletArgs.topic.event_ends_at);
    }

    get dateRange() {
        return this.eventEndedAt
            ? `${this._formattedDate(this.eventStartedAt)} → ${this._formattedDate(this.eventEndedAt)}`
