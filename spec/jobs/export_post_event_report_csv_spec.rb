              ...
              begin
                ...
                Zip::File.open(Discourse.store.path_for(upload)) do |zip_file|
                  ...
                  zip_file.each do |entry|
                    ...
                    expect(parsed_csv[1]).to eq([user_1.username, nil, invitee_1.created_at.to_s, invitee_1.updated_at.to_s])
                    invitee_2 = post_event.invitees.find_by(user_id: user_2.id)
                    expect(parsed_csv[2]).to eq([user_2.username, 'not_going', invitee_2.created_at.to_s, invitee_2.updated_at.to_s])
                  end
                end
